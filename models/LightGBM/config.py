"""Configuration specific to the LightGBM model."""

from training.config import (
    Config,
    ModelConfig,
    TrainingConfig,
    DataConfig,
    PathsConfig,
)

MODEL_DIR = "models/LightGBM"


def get_config() -> Config:
    """Return the complete configuration for LightGBM training."""
    return Config(
        model=ModelConfig(
            architecture="LightGBM",
            input_channels=5,
            auxiliary_dim=21,
            use_pca=True,
            pca_components=137,
            use_statistical_features=False,
            use_diff_features=False,
            n_estimators=1200,
            num_leaves=17,
            max_depth=10,
            min_child_samples=52,
            subsample=0.9993204662796431,
            colsample_bytree=0.7067610412460003,
            reg_alpha=6.722319751598018e-06,
            reg_lambda=0.00010860101209847184,
            class_weight="balanced",
        ),
        training=TrainingConfig(
            num_classes=2,
            batch_size=32,
            num_epochs=1,
            learning_rate=0.02639719696060605,
            patience=50,
            classification_threshold=0.5,
        ),
        data=DataConfig(
            augmentation_factor=0,
            num_workers=0,
            pin_memory=False,
        ),
        paths=PathsConfig(
            model_folder=f"{MODEL_DIR}/checkpoints",
            model_basename="lightgbm_model",
            experiment_name=f"{MODEL_DIR}/runs/lightgbm_v1",
        ),
        results_folder=f"{MODEL_DIR}/results",
    )
